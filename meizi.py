...
app.config['DATABASE_FILE'] = os.path.join(app.root_path, 'meizi.db')
...
down_path = op.join(op.dirname(__file__), 'meizi')
try:
    os.mkdir(down_path)
except OSError:
    pass
...
def downloadImage(url,page):
    path = op.join(op.dirname(__file__), 'meizi', page)
    try:
        os.mkdir(path)
    except:
        pass
...
ooxx = getHtml("http://jandan.net/ooxx", 10)
...
for page in range(kaishi,MaxPage+1):
    html = getHtml("http://jandan.net/ooxx/page-" + str(page), 10)
            ...
            try:
                ...
                if True:#support > unsupport:
                    if imgs.__len__()==1:
                        ...
                        if os.path.exists(op.join(op.dirname(__file__), 'meizi', str(page), imgsrc.split(u"/")[-1])):
                        ...
                        if pic.count() == 0:
                            ...
                            db.session.add(Meizi(foldername=str(page), picname=imgsrc.split(u"/")[-1], picurl=imgsrc, oo=support, xx=unsupport, nsfw=nsfw, myoo=0, myxx=0))
                        else:
